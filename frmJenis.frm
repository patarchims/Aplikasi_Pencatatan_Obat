...
Begin VB.Form frmJenis 
   ...
   Begin VB.Frame Frame1 
      Appearance      =   0  'Flat
      BackColor       =   &H00400000&
      Caption         =   "Insert Data Jenis Obat"
      ForeColor       =   &H00FFFFFF&
      Height          =   1095
      Left            =   0
      TabIndex        =   0
   ...
   Begin VSFlex7Ctl.VSFlexGrid Grid 
      ...
      FormatString    =   $"frmJenis.frx":0000
      ScrollTrack     =   0   'False
      ScrollBars      =   3
      ScrollTips      =   0   'False
      MergeCells      =   0
...
On Error GoTo SalahHapus
    ...
    sql = "Select * From tbjenis where id_jenis ='" & txtkode.Text & "'"
    ...
    If RsCek.RecordCount = 0 Then Exit Sub
        ...
        sql = "DELETE FROM tbjenis WHERE id_jenis ='" & txtkode.Text & "'"
...
End If
    ...
    sql = "INsert into tbjenis values('" & txtkode & "','" & txtnama & "')"
Else
    sql = "Update tbjenis set nm_jenis = '" & txtnama & "' where id_jenis = '" & txtkode.Text & "'"
End If
...
With Grid
Set RsCek = New ADODB.Recordset
sql = "Select *From   tbjenis   order by id_jenis desc"
RsCek.Open sql, Conn, 3, 4
If RsCek.RecordCount = 0 Then
    Urutan = "JNS" + "001"
    txtkode.Text = Urutan
Else
    Hitung = Right(RsCek!id_jenis, 3) + 1
    Urutan = "JNS" + Right("000" & Hitung, 3)
End If
txtkode.Text = Urutan
End With
End Sub
...
Next x
lblJlhData.Caption = "Jumlah = " & rsGrid.RecordCount & " Data Jenis Obat"
End Sub



Private Sub Form_Load()
Me.Caption = "Data Jenis Obat"
